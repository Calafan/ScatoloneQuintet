...
A card can carry at most one status: **Banned**, **Token** or **Jolly**. A card
with a status is set apart from the pool. It appears only in its own folder
(`Views/0_Banned/`, `Views/0_Token/`, `Views/0_Jolly/`) at any rating, it is
left out of the cube analysis, and `make-list` writes it in a section of its own.
...
| **Sacrifice** | An outlet you can feed your OWN creatures, artifacts or permanents at will — the half of the combo that makes a stolen creature worth taking. Not an edict (that empties their board), not a land, and not "as an additional cost to cast", which pays once. |
